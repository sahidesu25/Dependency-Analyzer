...
namespace CodeAnalysis
{
    ...
    public class FileMgr
    {
        ...
        private static bool recurse = true;
        ...
        public void findFilesForProject(string path)
        {
            if (patterns.Count == 0)
                addPattern("*.csproj");
            foreach (string pattern in patterns)
        ...
        public string[] getFilesForProjectList(string path, List<string> patterns)
        {
            FileMgr fm = new FileMgr();
            foreach (string pattern in patterns)
                fm.addPattern(pattern);
            fm.findFilesForProject(path);
            return fm.getFiles1().ToArray();
        }
